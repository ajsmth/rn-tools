...
let initialState: NavigationStoreState = {
  stacks: {
    lookup: {},
    ids: [],
  },
...
let storeBase = create<NavigationStoreState>(() => initialState);

let setState = storeBase.setState;
let getState = storeBase.getState;

export function setDebugModeEnabled(enabled: boolean) {
  setState((state) => Object.assign(state, { debugModeEnabled: enabled }));
}
...
function createNavigation() {
  ...
  let dispatch = (action: NavigationAction) =>
    reducer(getState(), action, { renderCharts });
}
...
type NavigationAction = StackActions;
    ...
    case "POP_SCREEN_BY_KEY": {
      ...
      let nextState = Object.assign(state, {
        stacks: {
          ids: state.stacks.ids,
          lookup: Object.assign({}, state.stacks.lookup, {
            [stackId]: stack.screens.filter((screenId) => screenId !== key),
          }),
        },
        screens: {
          ids: state.screens.ids.filter((id) => id !== key),
          lookup: Object.assign({}, state.screens.lookup, {
            [key]: undefined,
          }),
        },
...
// NavigationStoreState.subscribe((state) => {
//   if (state.debugModeEnabled) {
