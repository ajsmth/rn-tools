...
function* buildRegExps(context: any) {
  // let separator = context.tailwindConfig.separator;
  let separator = " ";
  // let variantGroupingEnabled = flagEnabled(context.tailwindConfig, 'variantGrouping')
  let variantGroupingEnabled = true;
  // let prefix =
  //   context.tailwindConfig.prefix !== ""
  //     ? regex.optional(
  //         regex.pattern([/-?/, regex.escape(context.tailwindConfig.prefix)])
  //       )
  //     : "";
...
function clipAtBalancedParens(input: string) {
  ...
  let depth = 0;
  let openStringTypes: any[] = [];

  // Find all parens, brackets, quotes, etc
  // Stop when we end at a balanced pair
  // This is naive and will treat mismatched parens as balanced
  // This shouldn't be a problem in practice though
  let matches: any = input.matchAll(SPECIALS);

  // We can't use lookbehind assertions because we have to support Safari
  // So, instead, we've emulated it using capture groups and we'll re-work the matches to accommodate
  matches = Array.from(matches).flatMap((match: any) => {
    // @ts-ignore
    const [, ...groups] = match;
