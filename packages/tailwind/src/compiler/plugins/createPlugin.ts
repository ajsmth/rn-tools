...
export default function createPlugin(config: any): any {
    ...
    for (let themeKey in themeConfig) {
      ...
      if (typeof themeValue === "object") {
        for (let platformKey in themeValue) {
          propertyKeys.forEach((propertyKey) => {
            ...
            propertyValueKeys.forEach((valueKey: string) => {
      ...
      // standard config
        ...
        propertyKeys.forEach((propertyKey) => {
          ...
          if (propertyValueKeys.length === 0) {
            styles[`${propertyKey}${key}`] = value;
          }

          propertyValueKeys.forEach((valueKey: any) => {
